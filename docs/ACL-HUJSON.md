...
Grants are default-deny **only if** the tailnet is not still using a legacy
`"acls": [{"src":["*"],"dst":["*:*"]}]` (or similar allow-all). **Replace
or remove that `*:*` member ACL**, then merge the fragment below. Leaving
`*:*` in place means students (and untagged keys) can still reach
`172.16.10.0/24`.

Omitting `172.16.10.0/24` from grants is the deny. Do not add it as a
`dst` for `tag:lab-student`.
...
| `tag:lab-access` | `autogroup:admin` | Per-pod dual-homed access LXC (subnet router) **only** — never mint this on student devices |
...
2. Portal minting prefers `tag:lab-student` only (never `tag:lab-access`).
   Until tagOwners exist, CT 145 / this repo fall back to an **untagged** key
